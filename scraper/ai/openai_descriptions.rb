...
def get_claude_description(book)
  begin
    ...
    throw "Claude API error for #{book.isbn}: #{body["error"]["message"]}" if body["error"]

    message = body.dig("content", 0, "text")
    ...
    if ((message.downcase.include?("clean up") || message.downcase.include?("cleanup")) && message.downcase.include?("merge")) || message.downcase.include?("no book descriptions")
